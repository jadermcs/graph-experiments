...
def main():
    ...
    parser = argparse.ArgumentParser(
                    ...
                    epilog = """Make count-based vector space from corpus.
    ...
    for sentence in sentences:
        for i, word in enumerate(sentence):
            lowerWindowSize = max(i-args.windowSize, 0)
            upperWindowSize = min(i+args.windowSize, len(sentence))
    ...
    # Save matrix
    Space(matrix=cooc_mat_sparse, rows=vocabulary, columns=vocabulary).save(args.outPath)
